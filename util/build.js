 ...
 * Állandók
 */
// a mester könyvtár helye a util-hoz viszonyítva
const MASTER    = process.cwd() + '/master/';
// az export könyvtár helye a util-hoz viszonyítva
const EXPORT    = process.cwd() + '/songbooks/';
// a végleges csomagolt fájlok helye
...
for (var i in files) {
  ...
  // kiolvassuk a fájl statisztikai adatait, hogy később az időbélyeget
  // be tudjuk állítani
  ...
  var data   = {
    title       : (select('//song:title[@lang="hu"][1]/text()', doc).join() != '')
                  ? select('//song:title[@lang="hu"][1]/text()', doc).join()
                  : select('//song:title[1]/text()', doc).join(),
  ...
  for (var j = 0; j < nodes.length; j++) {
    ...
    // módosítjuk az XML-t
    // ahol csak egy gyűjtemény van, nem kell változtatni semmit
    if (nodes.length > 1) {
    ...
    fs.writeFileSync(EXPORT + data.songbook + '/' + files[i]
      , (nodes.length > 1) ? copy.toString() : file);
    // szinkronizáljuk az új fájl időbélyegét a régihez
    fs.utimesSync(EXPORT + data.songbook + '/' + files[i]
      , stat.atime
      , stat.mtime);
 ...
 */
// globális jegyzék
var csv = json2csv({ data: index.global
  , fields: [ 'Cím'
  , 'Alternatív cím'
  , 'Eredeti cím'
  , 'Szöveg szerzői'
...
for (i in index.local) {
  csv = json2csv({ data: index.local[i]
    , fields: [ 'Gyűjtemény'
    , 'Sorszám'
    , 'Cím'
    , 'Alternatív cím'
 ...
 * Csomagolás
 */
// az aktuális futási könyvtárt megváltoztatjuk, hogy a zip csomagokban
// ne legyen benne a songbook mappa
process.chdir(EXPORT);
// listázzuk az összes export könyvtárat
var write = fs.readdirSync(process.cwd());
for (i in write) {
  if (!fs.lstatSync(write[i]).isDirectory())  {
    continue;
  }
  ...
  archive.add(write[i] + '.7z', write[i], {
    wildcards: [ '*.xml', '*.csv' ]
  })
  .progress(function(write) {
  })
  .catch(function(err) {
    console.error(err);
  });
  console.log('Csomagolva: ' + write[i]);
}
